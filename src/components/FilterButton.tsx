import { useState } from "react";

type Props = {
  text: string;
  onApply: (range: { min: number; max: number }) => void;
  initialMin: number;
  initialMax: number;
};

export default function PriceFilter({
  text,
  onApply,
  initialMin,
  initialMax,
}: Props) {
  const [min, setMin] = useState<number>(initialMin);
  const [max, setMax] = useState<number>(initialMax);
  const [open, setOpen] = useState(false);

  const handleApply = () => {
    onApply({ min, max });
    setOpen(false);
  };

  return (
    <div className="relative">
      <button
        onClick={() => setOpen(!open)}
        className="flex items-center gap-2 px-4 py-2 bg-gray-50 border border-gray-200 rounded-full text-sm active:scale-95 transition"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 640 640"
          className="w-4 h-4"
        >
          <path d="M96 128C78.3 128 64 142.3 64 160C64 177.7 78.3 192 96 192L182.7 192C195 220.3 223.2 240 256 240C288.8 240 317 220.3 329.3 192L544 192C561.7 192 576 177.7 576 160C576 142.3 561.7 128 544 128L329.3 128C317 99.7 288.8 80 256 80C223.2 80 195 99.7 182.7 128L96 128zM96 288C78.3 288 64 302.3 64 320C64 337.7 78.3 352 96 352L342.7 352C355 380.3 383.2 400 416 400C448.8 400 477 380.3 489.3 352L544 352C561.7 352 576 337.7 576 320C576 302.3 561.7 288 544 288L489.3 288C477 259.7 448.8 240 416 240C383.2 240 355 259.7 342.7 288L96 288zM96 448C78.3 448 64 462.3 64 480C64 497.7 78.3 512 96 512L150.7 512C163 540.3 191.2 560 224 560C256.8 560 285 540.3 297.3 512L544 512C561.7 512 576 497.7 576 480C576 462.3 561.7 448 544 448L297.3 448C285 419.7 256.8 400 224 400C191.2 400 163 419.7 150.7 448L96 448z" />
        </svg>
        {text}
      </button>
      {open && (
        <div className="absolute mt-2 w-64 bg-white border border-gray-100 shadow-lg rounded-xl p-3 z-50">
          <p className="text-sm font-semibold mb-3">Price Range</p>
          <div className="flex gap-2">
            <input
              type="number"
              value={min}
              onChange={(e) => setMin(Number(e.target.value))}
              className="w-full border rounded-lg px-2 py-1 text-sm"
              placeholder="Min"
            />

            <input
              type="number"
              value={max}
              onChange={(e) => setMax(Number(e.target.value))}
              className="w-full border rounded-lg px-2 py-1 text-sm"
              placeholder="Max"
            />
          </div>
          <button
            onClick={handleApply}
            className="w-full mt-3 bg-indigo-600 text-white py-2 rounded-lg text-sm active:scale-95 transition"
          >
            Apply Filter
          </button>
        </div>
      )}
    </div>
  );
}
